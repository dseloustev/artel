...
description: Create, generate, update, repair, trim, or split minimal AGENTS.md files for the host repo's root and confirmed nested modules using progressive disclosure. Use for missing, bloated, contradictory, or outdated agent instructions, monorepos, services, packages, module context, and portable agent guidance without assuming any fixed agent folder structure or toolchain.
---

# AGENTS.md Generator (Root + Nested, Portable)

## Goal
Maintain small, high-signal AGENTS.md files, generated entirely from the host repo's own content
— its manifests, docs, and existing `AGENTS.md` files, never a fixed stack assumption:
...
- Read `${CLAUDE_PLUGIN_ROOT}/skills/agents-md-generator/references/AGENTS_TEMPLATE_ROOT.md` only
  when creating or updating the root `AGENTS.md`.
- Read `${CLAUDE_PLUGIN_ROOT}/skills/agents-md-generator/references/AGENTS_TEMPLATE_MODULE.md`
  only after a directory is confirmed as a nested module that needs its own `AGENTS.md`.
...
- Detect language/tool markers, e.g.:
...
Both files are **host-repo content**: everything they say is derived from that repo's own
manifests, docs, and commands — never a template example, a project literal carried over from
elsewhere, or an assumption about which stack the host repo uses.
